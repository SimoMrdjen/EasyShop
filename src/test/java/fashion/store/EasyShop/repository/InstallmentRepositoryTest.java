...
class InstallmentRepositoryTest {
    ...
    void setUp() {
        ...
        purchaseContract1 = new PurchaseContract(null, customer, 5.00, 2.00, LocalDate.now(),
                new ArrayList<Installment>());
        installment = repository.save(new Installment(purchaseContract, InstallmentOrdinal.FIRST,
                20.00, LocalDate.now().plusMonths(1)));
        installment2 = repository.save(new Installment(purchaseContract1, InstallmentOrdinal.FIRST,
    ...
    void shouldreturnListWhenFindAllByPurchaseContract_Customer_Id() {
      List<Installment> insts = repository.findAllByPurchaseContract_Customer_Id(1L);
        assertThat(insts).usingDefaultElementComparator().
                isEqualTo(List.of(installment,installment2));    
    }
}
